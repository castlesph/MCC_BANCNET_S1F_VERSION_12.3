...
//#endif
	...
	CRYPTO_ctr128_encrypt(felicaCmdParams, payload, felicaCmdParamsLen, &_aes_key, _ctr_block, _iv, &_num);
    ...
    CRYPTO_ctr128_encrypt(_raw_mac, mac, 8, &_aes_key, _ctr_block ,_iv, &_num);
}
...
long _decrypt_sam_response(unsigned char samResponse[], unsigned int samResLen, unsigned char plainPackets[]) {
	...
	CRYPTO_ctr128_encrypt(&samResponse[1+1+3+4], plainPackets, _enc_data_len, &_aes_key, _ctr_block, _iv, &_num);
#ifdef DEBUG
	...
	CRYPTO_ctr128_encrypt(&samResponse[samResLen-8], _received_mac, 8, &_aes_key, _ctr_block, _iv, &_num);
