...
int inPackSendAndUnPackData(TRANS_DATA_TABLE *srTransPara, int inTransCode)
{
    ...
    if((srTransPara->byPackType == LOG_ON) && (strTCT.fDemo == VS_FALSE))
    {
...
void vdModifyBitMapFunc(int inTransCode, int *inBitMap)
{
    ...
    {
       vdCTOSS_SetBitMapOn(inBitMap, 14);
	   vdCTOSS_SetBitMapOff(inBitMap, 43);
    }
	if ((srTransPara->byPackType == REVERSAL) && (isCashOutFlag == FALSE))
	{
                             ...
                             unsigned char* uszReceData)
{
	...
	char uszPrntSendData[ISO_SEND_SIZE+1];
	int inPrntLen = 0;
	...
	vdDebug_LogPrintf("inAmexETHDirect[%d]", strSSL.inAmexETHDirect);
    ...
    else
    {
		 if(strTCT.fTPSec == TRUE && strHDT.fHostTPSec == TRUE)
		 {
			...
			//back the ISO for printing,cause HTTPS post mode can't print normal ISO
			memset(uszPrntSendData,0x00,sizeof(uszPrntSendData));
			inPrntLen = inSendLen;
			memcpy(uszPrntSendData,uszSendEcnryptedData,inPrntLen);
			DebugAddHEX("uszPrntSendData",uszPrntSendData,inPrntLen);
			//For Post
			if (strSSL.inAmexETHDirect == 1)
			{
				vdCTOSS_PackAmexETHDirect(uszSendEcnryptedData,&inSendLen);
				vdDebug_LogPrintf("vdCTOSS_PackAmexETHDirect sendlen[%d]", inSendLen);
			}
            DebugAddHEX("AFTER POST ",uszSendEcnryptedData,inSendLen);
			//end for POST
			...
			// Parse ISO Send
			 ...
			 {	
	            ...
	            vdPrintISOOption("TX",uszSendData, inPrntLen);

				inPrint("*ENCRYPTED PACKET*");
	            vdPrintISOOption("TX",uszPrntSendData, inPrntLen);
			 }

			 if(inResult == d_OK && fISOLog)
			 {
			 	inPrint("*CLEAR PACKET*");
				vdLogISOOption("TX",uszSendData, inPrntLen);

				inPrint("*ENCRYPTED PACKET*");
				vdLogISOOption("TX",uszPrntSendData, inPrntLen);
			 }
		 ...
		 else
        {
			...
			memset(uszPrntSendData,0x00,sizeof(uszPrntSendData));
			inPrntLen = inSendLen;
			memcpy(uszPrntSendData,uszSendData,inPrntLen);
			DebugAddHEX("uszPrntSendData",uszPrntSendData,inPrntLen);
			
			 //For Post
			if (strSSL.inAmexETHDirect == 1)
			{
				vdCTOSS_PackAmexETHDirect(uszSendData,&inSendLen);
				vdDebug_LogPrintf("vdCTOSS_PackAmexETHDirect sendlen[%d]", inSendLen);
			}
            DebugAddHEX("AFTER POST ",uszSendData,inSendLen);
			//end for POST
             ...
             if((strTCT.inISOLogger == TRUE) && (inResult == d_OK)){
                  inSaveISOLog(VS_TRUE, uszPrntSendData, inPrntLen);
             }
        }
    }
    ...
    else
    {
        ...
        {   
			...
			if (strSSL.inAmexETHDirect == 1)
			{
				vdCTOSS_UnPackAmexETHDirect(uszReceData,&inResult);
				vdDebug_LogPrintf("vdCTOSS_UnPackAmexETHDirect inReceLen[%d]", inResult);
				vdPCIDebug_HexPrintf("receive data",uszReceData,inResult);
			}
...
int inCheckIsoHeaderData(char *szSendISOHeader, char *szReceISOHeader)
{
	...
	//post mode don't have header
	if (strSSL.inAmexETHDirect == 1)
	{
		//post mode can't compare 
		return ST_SUCCESS;
	}
...
//    vduiClearBelow(8); // patrick fix code 20140421
    ...
    {
		if((srTransRec.byTransType != BAL_INQ) && (srTransRec.byTransType != LOG_ON))
		{
...
int inAnalyseReceiveData(void)
{
	...
	if ((srTransPara->byEntryMode == CARD_ENTRY_ICC) && 
            ((srTransPara->byTransType == SALE) || (srTransPara->byTransType == PRE_AUTH) || (srTransPara->byTransType == BAL_INQ) || (srTransPara->byTransType == CASH_OUT) /*|| (srTransPara->byTransType == REFUND)*/)
           )
	{
...
void vdGetTimeDate(TRANS_DATA_TABLE *srTransPara)
{
	...
	|| srTransPara->byPackType == REVERSAL
	|| srTransPara->byPackType == CASH_OUT)		 
    {
        CTOS_RTC SetRTC;
        BYTE szCurrentTime[20];
...
int inUnPackIsoFunc54(TRANS_DATA_TABLE *srTransPara, unsigned char *uszUnPackBuf)
{
	...
	if(srTransPara->byTransType == BAL_INQ)
	{
		...
		memcpy(strtemp, &uszUnPackBuf[10], 12);
		memset(srTransPara->szTotalAmount, 0x00, sizeof(srTransPara->szTotalAmount));
