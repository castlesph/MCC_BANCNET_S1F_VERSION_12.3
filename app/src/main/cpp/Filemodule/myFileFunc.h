...
typedef struct
{
	...
	BYTE szEMVDebitTermCapability[11 + 1];
	BOOL fPrintBalInqCopy;
	UINT inERMMaximum;
	BYTE byERMInit;
	...
	INT inGPRSFallback;
	BOOL fTLESec;
	BYTE szServiceFee[15]; // For Bancnet CashOut -- sidumili
	BYTE szCashoutMinAmt[15]; // For Bancnet Cashout minimum amount -- sidumili
	BYTE szCashoutMaxAmt[15]; // For Bancnet Cashout maximum amount -- sidumili
...
typedef struct
{
	...
	//fix for crash - add +1 in sizeof szPassword
    BYTE    szPassword[6+1];	
	UINT    inFailedREV;
	BOOL	fBatchNotEmpty;
} STRUCT_MMT;
...
{	
	int    inAmexETHDirect;
    BYTE   szAMEXRTIND[50];
    BYTE   szAMEXORIGIN[100];
    BYTE   szAMEXCOUNTRY[50];
    BYTE   szAMEXMESSAGE[50];
    BYTE   szAMEXREGION[50];
    BYTE   szAMEXCONTENT_TYPE[50];
    BYTE   szAMEXHOST[50];
    BYTE   szAMEXCACHE_CONTROL[50];
    BYTE   szAMEXCONNECTION[50];
    BYTE   szDummy1[50];
    BYTE   szDummy2[50];
    BYTE   szDummy3[50];
} STRUCT_SSL;
...
int inMyFile_TCUploadFileReadLIFO(int *inSeekCnt, STRUCT_ADVICE *strAdvice, TRANS_DATA_TABLE *transData);
...
STRUCT_EMVT     strEMVT_Temp; // Force to read record 7(BANCNET) -- sidumili
