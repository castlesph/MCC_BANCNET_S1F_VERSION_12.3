...
USHORT printDateTime(char *ptrDate, char *ptrTime, BOOL fTrans)
{
	...
	if(atoi(srTransRec.szYear) > 20)
	{
		sprintf(szYear, "%02d", atoi(srTransRec.szYear));
...
{	
	...
	if(CUSTOMER_COPY_RECEIPT == page)
	{
        ...
        memset(szStr, 0x00, d_LINE_SIZE);
        sprintf(szStr, "TRAN TYPE             %s", szTemp);
        result=inPrint(szStr);
        #endif
        ...
        memset(szStr, 0x00, d_LINE_SIZE);
        sprintf(szStr, "CARD #                %s", baTemp);
        result=inPrint(szStr);

		//aaronnino fix on bancnet issue Bank Name not displayed in the receipt 2 of 2 start
		//if (srTransRec.fDebit == TRUE) 
		//{
		    memset(szTemp, ' ', d_LINE_SIZE);
		    strcpy(szTemp,strCDT.szBank_Mnemonic);
		    sprintf(szStr, "BANK NAME             %s", szTemp);
            result=inPrint(szStr);
		//}
		//aaronnino fix on bancnet issue Bank Name not displayed in the receipt 2 of 2 end
		#endif
        ...
        memset(szTemp1, 0x00, d_LINE_SIZE);
        memset(szTemp2, 0x00, d_LINE_SIZE);
        sprintf(szTemp1, "REF # %s", srTransRec.szRRN); szTemp1[18]=0;
	...
	else if(MERCHANT_COPY_RECEIPT == page || BANK_COPY_RECEIPT == page)
	{
		...
		//aaronnino fix on bancnet issue Bank Name not displayed in the receipt 1 of 2 start
		//if (srTransRec.fDebit == TRUE)
		//{
              memset(szTemp, ' ', d_LINE_SIZE);
		      strcpy(szTemp,strCDT.szBank_Mnemonic);
              ...
              result=inPrint(szStr);
		//}
		//aaronnino fix on bancnet issue Bank Name not displayed in the receipt 1 of 2 end
		#endif
        ...
        memset(szTemp2, 0x00, d_LINE_SIZE);
		//wub_hex_2_str(srTransRec.szBatchNo,szTemp1,3);
		wub_hex_2_str(srTransRec.szInvoiceNo, szTemp2, INVOICE_BCD_SIZE); /*Coreware*/
		srTransRec.szBancnetBatchNo[6]=0x00; /*Coreware*/
        memset(szStr, 0x00, d_LINE_SIZE);
        //sprintf(szStr, "BATCH # %s        INVOICE # %s", szTemp1, szTemp2); /*Coreware*/
        sprintf(szStr, "BATCH # %s        INVOICE # %s", srTransRec.szBancnetBatchNo, szTemp2); /*Coreware*/
...
USHORT ushCTOS_ReprintLastSettleReport(void)
{
	...
	DebugAddHEX("ushCTOS_ReprintLastSettleReport srAccumRec.szYear", srAccumRec.szYear, DATE_BCD_SIZE);
	DebugAddHEX("ushCTOS_ReprintLastSettleReport srAccumRec.szDate", srAccumRec.szDate, DATE_BCD_SIZE);
	DebugAddHEX("ushCTOS_ReprintLastSettleReport srAccumRec.szTime", srAccumRec.szTime, TIME_BCD_SIZE);
	...
	DebugAddHEX("ushCTOS_ReprintLastSettleReport srTransRec.szYear", srTransRec.szYear, DATE_BCD_SIZE);
	DebugAddHEX("ushCTOS_ReprintLastSettleReport srTransRec.szDate", srTransRec.szDate, DATE_BCD_SIZE);
	DebugAddHEX("ushCTOS_ReprintLastSettleReport srTransRec.szTime", srTransRec.szTime, TIME_BCD_SIZE);
...
void vdRestoreReportTime(void)
{
	...
	//wub_str_2_hex(szgTempDate, srTransRec.szDate, DATE_ASC_SIZE);
    //wub_str_2_hex(szgTempTime, srTransRec.szTime, TIME_ASC_SIZE);
	...
	DebugAddHEX("vdRestoreReportTime srTransRec.szDate", srTransRec.szDate, DATE_BCD_SIZE);
	DebugAddHEX("vdRestoreReportTime: srTransRec.szTime", srTransRec.szTime, TIME_BCD_SIZE);
...
void vdPrintDateTimeAlignLeftRight(void)
{
	...
	memcpy(szgTempDate, szTemp1, DATE_ASC_SIZE);
	memcpy(szgTempTime, szTemp2, TIME_ASC_SIZE);
...
void vdPrintAcntType(void)
{
	...
	inPrintLeftRight("ACCT TYPE:", szStrRight, 46);
}
...
USHORT printSettleDateTime(BOOL fLastSettle)
{
	BYTE   EMVtagVal[64];
	USHORT EMVtagLen;
	int result;
	...
	vdDebug_LogPrintf("--printDateTime--");
	vdDebug_LogPrintf("szTranYear[%s]", srTransRec.szTranYear);
	vdDebug_LogPrintf("fTrans[%d]", fLastSettle);
