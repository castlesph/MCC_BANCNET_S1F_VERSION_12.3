...
// #define LOCAL_RECEIPT_FILE			"/data/data/com.Source.S1_MCC.MCC/logo.bmp"
// #define LOCAL_RECEIPT_FILE_NOLOGO	"/data/data/com.Source.S1_MCC.MCC/nologo.bmp"
#define LOCAL_RECEIPT_FILE			"/data/data/com.Source.S1_BANCNET.BANCNET/fs_data/logo.bmp"
#define LOCAL_RECEIPT_FILE_NOLOGO	"/data/data/com.Source.S1_BANCNET.BANCNET/fs_data/nologo.bmp"
...
unsigned long g_ulSettleTotal = 0;

void vdResetERMEmail()
{
	memset(g_szEmail,0x00,sizeof(g_szEmail));
}

void vdSetERMEmail(char *szemail)
{
	memset(g_szEmail,0x00,sizeof(g_szEmail));
...
{
	g_ulSettleTotal = ulTotalAmt;
}

unsigned long ulCTOSS_ERM_GetSettleTotal(void)
{
	return g_ulSettleTotal;
}
...
int inCTOSS_ERMCallbackDisplay(char *Appname, int IPC_EVENT_ID, BYTE *bOutBuf, USHORT *usOutLen)
{
	BYTE bInBuf[512];
	USHORT usInLen = 0;
	USHORT usResult;
	BYTE *ptr = NULL;
	STRUCT_CALLBACK_DISPLAY szCallbackDisp;


	vdDebug_LogPrintf("inCTOSS_ERMCallbackDisplay........"); 
    vdDebug_LogPrintf("usOutLen=[%d],bOutBuf=[%s]",*usOutLen,bOutBuf);
	if (memcmp(bOutBuf,SHARE_ERM_SUB_IPC,3) == 0)
	{
		  if (memcmp(&bOutBuf[3],SHARE_ERM_SHOWDISPLAY,1) == 0)
		  {
			  vdDebug_LogPrintf("SHARE_ERM_SHOWDISPLAY..........");
			  memset(&szCallbackDisp, 0x00, sizeof(STRUCT_CALLBACK_DISPLAY));
			  memcpy(&szCallbackDisp,&bOutBuf[4],sizeof(STRUCT_CALLBACK_DISPLAY));

			  vdDebug_LogPrintf("X=[%d],Y=[%d]",szCallbackDisp.X,szCallbackDisp.Y);
			  vdDebug_LogPrintf("szDisplay=[%s]",szCallbackDisp.szDisplay);
			  // CTOS_LCDTPrintXY(szCallbackDisp.X,szCallbackDisp.Y, szCallbackDisp.szDisplay);

			  usInLen = 0;
			  memset(bInBuf, 0x00, sizeof(bInBuf));
			  strcpy(bInBuf,SHARE_ERM_SUB_IPC);
			  strcat(bInBuf,SHARE_ERM_SHOWDISPLAY);
			  usInLen = strlen(bInBuf);
			
			  vdDebug_LogPrintf("usInLen=[%d],str=[%s]",usInLen,bInBuf);

			  memset(bOutBuf, 0x00, sizeof(bOutBuf));	 
			  usResult = inMultiAP_RunIPCCmdTypes(Appname, IPC_EVENT_ID, bInBuf, usInLen, bOutBuf, usOutLen);
			  vdDebug_LogPrintf("inMultiAP_RunIPCCmdTypes usResult=[%d]",usResult);
			  vdDebug_LogPrintf("usOutLen=[%d],bOutBuf=[%s]",*usOutLen,bOutBuf);
			  
			  if (d_OK == usResult)
			  	usResult = inCTOSS_ERMCallbackDisplay(Appname, IPC_EVENT_ID, bOutBuf, usOutLen);
			  else
			  	return d_NO;
		  }
...
//receipt logo will upload in whole receipt
	...
	chmod((char *)ERM_RECEIPT_FILE, 0777);
	inERMAP_Database_BatchDelete();
...
USHORT usCTOSS_Erm_Initialization(void)
{
	...
	if (usResult == d_OK)
	{
		usResult = inCTOSS_ERMCallbackDisplay("com.Source.SHARLS_ERM.SHARLS_ERM", d_IPC_CMD_ERM_INITIALIZATION, bOutBuf, &usOutLen);
		vdDebug_LogPrintf("inCTOSS_ERMCallbackDisplay usResult=[%d]",usResult);
		if (usResult == d_OK)
		{
...
USHORT usCTOSS_Erm_FromReceipt(void)
{
	...
	if (usResult == d_OK)
	{
		usResult = inCTOSS_ERMCallbackDisplay("com.Source.SHARLS_ERM.SHARLS_ERM", d_IPC_CMD_ERM_FORM_RECEIPT, bOutBuf, &usOutLen);
		vdDebug_LogPrintf("inCTOSS_ERMCallbackDisplay usResult=[%d]",usResult);
		if (usResult == d_OK)
		{
		if (bOutBuf[1] == d_SUCCESS)
...
USHORT usCTOSS_Erm_SendAlert(void)
{
	...
	usResult = inMultiAP_RunIPCCmdTypes("com.Source.SHARLS_ERM.SHARLS_ERM", d_IPC_CMD_ERM_SEND_ALERT, bInBuf, usInLen, bOutBuf, &usOutLen);
    
	vdDebug_LogPrintf("usCTOSS_Erm_SendAlert usOutLen[%d] bOutBuf[%s]",usOutLen, bOutBuf);
    return usResult;
}
USHORT usCTOSS_Erm_UploadReceipt(BOOL fSettle)
{
    ...
    if (usResult == d_OK)
	{
		usResult = inCTOSS_ERMCallbackDisplay("com.Source.SHARLS_ERM.SHARLS_ERM", d_IPC_CMD_ERM_UPLOAD_RECEIPT, bOutBuf, &usOutLen);
		vdDebug_LogPrintf("inCTOSS_ERMCallbackDisplay usResult=[%d]",usResult);
		if (usResult == d_OK)
		{
		if (bOutBuf[1] == d_SUCCESS)
...
int inCTOSS_ERM_CheckSlipImage(void)
{
	...
	//temporary return first, cause call ERM, can't enter anything
	return d_OK;
...
int inDeleteGzipFile(unsigned char *pchFileName)
{
	...
	int inRetVal = 0;
	FILE  *fPubKey;
	unsigned char uszFileName[128 + 1];
...
int inCTOSS_ERM_CardinfoENQ(void)
{
	...
	if(d_OK == usResult)
	{
		usResult = inCTOSS_ERMCallbackDisplay("com.Source.SHARLS_ERM.SHARLS_ERM", d_IPC_CMD_ERM_CARDINFO_ENQ, bOutBuf, &usOutLen);
		vdDebug_LogPrintf("inCTOSS_ERMCallbackDisplay usResult=[%d],bOutBuf[1]=[%d]",usResult,bOutBuf[1]);
		if (usResult == d_OK)
		{
