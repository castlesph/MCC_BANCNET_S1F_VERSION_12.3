...
int inCTOS_UpdateAccumTotal(void)
{
    ...
    switch(srTransRec.byTransType)
    {
		...
		case CASH_OUT:
			...
			// Service Fee
			srAccumRec.stBankTotal[inTranCardType].stCardTotal[srTransRec.IITid].ulServiceFeeTotalAmount= srAccumRec.stBankTotal[inTranCardType].stCardTotal[srTransRec.IITid].ulServiceFeeTotalAmount+ atol(szServiceFeeAmt);
			srAccumRec.stBankTotal[inTranCardType].stHOSTTotal.ulServiceFeeTotalAmount= srAccumRec.stBankTotal[inTranCardType].stHOSTTotal.ulServiceFeeTotalAmount+ atol(szServiceFeeAmt);
			
			
			if((srTransRec.byTransType == CASH_OUT) && (srTransRec.byEntryMode == CARD_ENTRY_ICC))
				srAccumRec.stBankTotal[inTranCardType].usEMVTCCount++;
			break;	
        default:
            break;
