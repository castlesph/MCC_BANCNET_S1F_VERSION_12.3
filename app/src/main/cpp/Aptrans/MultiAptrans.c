...
int inMultiAP_RunIPCCmdTypesEx(char *Appname, int IPC_EVENT_ID, BYTE *inbuf, USHORT inlen, BYTE *outbuf, USHORT *outlen)
{
	...
	// patrick disable sharls erm by return success start 20190319 start
	{
		if (strcmp(Appname, "com.Source.SHARLS_ERM.SHARLS_ERM") == 0)
		{
			vdDebug_LogPrintf("inMultiAP_RunIPCCmdTypes strcmp=[%d][%s],[%x]",pid, Appname,IPC_EVENT_ID);
			outbuf[0] = IPC_COMM_SUCC;
			*outlen = 1;
			return d_OK;
		}
		
	}
	// patrick disable sharls erm by return success start 20190319 end
