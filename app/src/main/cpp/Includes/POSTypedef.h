...
#define LOCAL_PATH 					"/data/data/com.Source.S1_BANCNET.BANCNET/"
#define UNFORK_AP_FILE			"unforkap.ini"

//#define LOGO_PATH 					"/data/data/com.Source.S1_BANCNET.BANCNET/fs_data/"  // S1F2 support for download logo via ctms 
...
#define LOY_BAL_INQ			130
#define LOY_REDEEM_5050		131
#define LOY_REDEEM_VARIABLE	132
#define LOY_REDEEM_REVERSAL 133 

#define INSTALLMENT_SALE 134 

#define TERM_REG 135

#define CARD_VERIFY 136

#define CITAS_SERVICES	 	 137
#define MERCHANT_SERVICES	 138
#define MERCHANT_ORIENT	 	 139
#define PRINT_CTMS_RPT   	 160
#define CASH_OUT	 	 	 161
...
#define ETHERNET_MODE       1
#define GPRS_MODE           2    
#define COM1_MODE           3//for COM&USB communication
#define COM2_MODE           4
#define USB_MODE           	5
...
#define CVV2_BYTES                 4
#define CARD_HOLD_NAME_DIGITS      31
...
typedef struct
{
    ...
    BYTE T9F41[4];    // Original Trace No (Field 11)
    BYTE T9F53;        // TCC,
...
typedef struct
{
    ...
    emvinfo_t   stEMVinfo;
	BYTE 		szTerms[2+1];
	BYTE 		szFactorRate[7+1];
	BYTE        szMoAmort[12+1];
	BYTE        szTotalInstAmnt[12+1];
	BYTE 		szPromo[2+1];	
	BYTE 		szInstallmentData[81+1];
	BOOL 	    fInstallment;
	int         inAccountType;
	BOOL 	    fLogon;
	BYTE 		szProductCode[16+1];
	BYTE 		szCustomerNo[16+1];
	BYTE 		szLoyaltyData[105+1];
	BOOL 	    fLoyalty;
	BOOL 		fDebit;	
	BOOL 		fCredit;
	BYTE        szRedeemAmount[12+1];
    BYTE        szOrgTipAmount[AMT_BCD_SIZE+1];
	BYTE 		szPromoLabel[20+1];
	BYTE 		szBancnetBatchNo[6+1];
	BYTE 		szBancnetTraceNo[6+1];
	BYTE		szBank_Mnemonic[19+1];
    BYTE szResponseText[40+1];
	BYTE szMerchantName[69+1];
	BOOL fECRTxnFlg;
	BYTE byEMVReversal;
	BYTE szTerminalCapability[11 + 1];

	// Mercury Requirements -- sidumili
	BYTE szECRMessage1[40+1];
	BYTE szECRMessage2[40+1];
	BYTE szECRMessage3[40+1];
	BYTE szECRMessage4[40+1];
	BYTE szTranYear[4 + 1];

	BOOL fAutoSettleFlag;
	BYTE szServiceFee[AMT_BCD_SIZE+1];
...
typedef struct
{
    BYTE        szISOLogData[10000];
	BYTE        szISOLogTransType[50];
	BYTE        szISOLogDateTime[50];
    ULONG       ulSavedIndex;// kobea added  
		 
}TRANS_ISOLOG_TABLE;
	
TRANS_ISOLOG_TABLE srTransISOLog;
...
#define BANCNET_HOST 1
#define DINERS_HOST 6

// For Password Length
#define PWD_MAX_LEN       6
#define PWD_MIN_LEN       4

// For message type image -- sidumili
...
#define ECR_STAN_ERR            "NM"
#define ECR_APPROVED_RESP		"APPROVED"
#define ECR_OPER_CANCEL_RESP    "OPERATOR CANCEL"
#define ECR_EXPIRED_CARD_RESP	"CARD EXPIRED"
#define ECR_AMOUNT_ERR_RESP	    "OUT OF RANGE"
#define ECR_RECORD_VOIDED       "RECORD VOIDED"
#define ECR_STAN_NOT_MATCH      "STAN NOT MATCH"
