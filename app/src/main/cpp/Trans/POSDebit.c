...
int inCTOS_DebitSaleFlowProcess(void)
{
  	...
  	if((strTCT.fAutoDebitLogon == TRUE) && (strTCT.fDemo==VS_FALSE))
	{
		...
		BYTE szCurrentTime[4+1];
		BYTE szCurrentTimeBCD[2+1];
		//end of fix
		CTOS_RTC SetRTC;

        //save selcted merchant - for multi merchant logon
        int inMerchatNum = 0;
        inMerchatNum=strMMT[0].MITid;
        //end test
		...
		if(memcmp(strTCT.szLogonDate, szCurrentTimeBCD, 2) != 0)
		{
            ...
            //reload the original merchant number selected which was changed if multi-merchant logon was performed
            inMMTReadRecord(strHDT.inHostIndex,inMerchatNum);
...
int inCTOS_DebitBalanceInquiryFlowProcess(void)
{
	...
	{
	    //fix for crash - add 1 for null terminator
		//BYTE szCurrentTime[4];
		//BYTE szCurrentTimeBCD[2];
		BYTE szCurrentTime[4+1];
		BYTE szCurrentTimeBCD[2+1];
...
int inCTOS_DebitCashOutFlowProcess(void)
{
	...
	vdDebug_LogPrintf("strTCT.fAutoDebitLogon:(%d)", strTCT.fAutoDebitLogon);
 	...
 	if(strTCT.fAutoDebitLogon == TRUE)
	{
	    //fix for crash - add 1 for null termninator
		//BYTE szCurrentTime[4];
		//BYTE szCurrentTimeBCD[2];
		BYTE szCurrentTime[4+1];
		BYTE szCurrentTimeBCD[2+1];
