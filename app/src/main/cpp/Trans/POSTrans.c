...
int inCTOS_WaveGetCardFields(void)
{
	...
	//fix for crash - change szCatgCode size from 3 to 10
	BYTE szOtherAmt[12+1],szTransType[2+1],szCatgCode[10+1],szCurrCode[3+1];
...
int inCTOS_FirstGenACGetAndSaveEMVData(void)
{
    ...
    unsigned char szTransSeqCounter[8+1];
...
void vdGetCRC(void)
{
	...
	if ((file = fopen ("V5S_BANCNET", "rb")) != NULL)
	{
		...
		if (fHD)
		{
			put_env_char("V5S_BANCNET_CRC","BEF7FF2A8");
		}
		else
		{
...
#define MGTAMEXCGI1 "/TermMsgFwd/TermMsgHandler.ashx"
char * AmexSSL_getPOSTHdr(void)
{
	char  StrMethod[1000] ,*tmp;
...
void AmexSSL_addHdr3 ( char **data , const char * appended ,int inSize)
{ 
     char* tmp = (char *)malloc( strlen(*data) + inSize +1); //for space and \0
	 char * privot = tmp + strlen(*data);
     strcpy( tmp , *data ); // first copy all data into new buffer

     memcpy( privot , appended , inSize); // secound appends data into a header
...
void vdCTOSS_PackAmexETHDirect(unsigned char* uszSendData,int *inSendLen)
{
	...
	vdDebug_LogPrintf("inSendSize = [%d]",inSendSize);
    ...
    {  
        case 1:
			PtrSend[0] = ((inSendSize & 0x0000FF00) >> 8);
    		PtrSend[1] = (inSendSize & 0x000000FF);
            break;
        case 3:
			inSendSize += 2;//this follow inCTOSS_COMM_Send function pack
			vdDebug_LogPrintf("inSendSize = [%d]",inSendSize);
			PtrSend[0] = ((inSendSize & 0x0000FF00) >> 8);
    		PtrSend[1] = (inSendSize & 0x000000FF);  
            break;
	...
	memcpy(&PtrSend[2],uszSendData,inSendSize);
	//vdPCIDebug_HexPrintf("PtrSend",PtrSend,inSendSize+2);
	...
	sprintf(LengthData,"%d",inSendSize+2);
	...
	*inSendLen = strlen(data);
	...
	//vdPCIDebug_HexPrintf("PtrSend",PtrSend,inSendSize+2);
	AmexSSL_addHdr3( &data ,PtrSend, inSendSize+2);
	
	//*inSendLen = strlen(data);
	*inSendLen += inSendSize+2;
	vdDebug_LogPrintf("inSendLen = [%d]",*inSendLen);
    memcpy(uszSendData,data,*inSendLen);
	uszSendData[*inSendLen] = 0x00;
	//vdPCIDebug_HexPrintfEx("uszSendData",uszSendData,*inSendLen);
...
void vdCTOSS_UnPackAmexETHDirect(unsigned char* uszReceData,int *inReceLen)
{
	...
	PtrReceive += inRetval-1;

	if(((*PtrReceive == '\n')&&(*(PtrReceive-1) == '\r')) || ((*PtrReceive == '\r')&&(*(PtrReceive-1) == '\n')))
	{
		*PtrReceive = 0x00;
		PtrReceive--;
		*PtrReceive = 0x00;
	...
	while(1)//((*PtrReceive != 0x0A)&&(*(PtrReceive-1) != 0x0D))
	{
		vdDebug_LogPrintf("[%02x][%02x]",*PtrReceive,*(PtrReceive-1));
		if(((*PtrReceive == '\n')&&(*(PtrReceive-1) == '\r')) || ((*PtrReceive == '\r')&&(*(PtrReceive-1) == '\n')))
		{
			vdDebug_LogPrintf("[%02x][%02x]",'\n','\r');
			break;
		}
		PtrReceive--;
		tmplen++;
	}
	vdDebug_LogPrintf("tmplen=[%d]",tmplen);
	vdPCIDebug_HexPrintf("PtrReceive",PtrReceive,tmplen);
	#else
	//looks have problem for ERM,
#if 0
[SHARLS_ERM]<12062>[V5IsoFunc.c:946]: receive data=[HTTP/1.1 200 OK
Cache-Control: private
Content-Type: text/plain
Server: Microsoft-IIS/8.5
X-AspNet-Version: 4.0.30319
X-Powered-By: ASP.NET
Date: Fri, 06 Oct 2017 08:47:43 GMT
Content-Length: 40
00116001240123001100000000020000003030]
[SHARLS_ERM]<12063>[POSTrans.c:561]: inRetval=243
[SHARLS_ERM]<12064>[POSTrans.c:594]: LENGTH BYTE BUFFER = [121],[243]
[SHARLS_ERM]<12065>[POSTrans.c:349]: NumTemp<NOCHANGE> = 0
[SHARLS_ERM]<12066>[POSTrans.c:603]: LENGTH BYTE BUFFER = [121],[119]
#endif
	//fix Bug #1075, Logical Error
	n_offset = inRetval-1;
    while(n_offset > 0 && PtrReceive[n_offset] != '\n')
    {
        n_offset--;
    }
    inLen = (n_offset+1)/2;
	#endif
#if 0	
	//this is for ERM reponse
	vdDebug_LogPrintf("tmplen=[%d]",strlen(PtrReceive));
	...
	vdDebug_LogPrintf("LENGTH BYTE BUFFER = [%d],[%d]",inLen,*inReceLen );
	
	if (inLen > 0)
	{
	AciiToByte_For_SSL(szReceive, PtrReceive);
	//memcpy(uszReceData, szReceive, inLen);
	*inReceLen = inLen - 2;//delete 2 bytes length
	memcpy(uszReceData, szReceive+2, *inReceLen);
	...
	memcpy(uszReceData, PtrReceive, tmplen);	
	*inReceLen = tmplen;
#endif
	return  ;
}
