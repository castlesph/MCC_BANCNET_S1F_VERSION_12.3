...
int inCTOS_GetTxnBaseAmount(void)
{
    ...
    BYTE key;
	//adjust size of sztemp - fix for possible crash - S1F
    //BYTE szTemp[20];
    BYTE szTemp[30];
    //end of fix
    BYTE baAmount[20];
...
int inCTOS_CustComputeAndDispTotal(void)
{
    ...
    */
	...
	if (ret == d_USER_CANCEL) /*timeout and key cancel*/
	{
...
int inCTOS_DisplaySettleBatchTotal(BYTE byTransType, BOOL fConfirm)
{
	...
	if (isCashOutFlag)
	{
		...
		if (ulCashOutTotalAmount > 0)
		{
			...
			if (isCheckTerminalMP200() == d_OK || isCheckTerminalNonTouch == d_OK)
			{
				...
				inDisplayLeftRight(2, "CASHOUT",szDisplayBuf,20);
			}
			else
			{
				sprintf(szDisplayBuf, "   %s   %s", strCST.szCurSymbol, szAmtBuf); 				
    ...
    */
	...
	else
	{
		...
		vdDebug_LogPrintf("SATURN inCTOS_DisplaySettleBatchTotal 2");
		if (isCheckTerminalMP200() == d_OK || isCheckTerminalNonTouch == d_OK)
		{			
      ...
      {	      				      	
		...
		if (isCashOutFlag)
		{
			sprintf(szCount, "%03d", usCashOutCount);
			sprintf(szTemp, "SETTLEMENT|CASHOUT:|%s|", szCount);
		}
		else
		{
      ...
      else
      {
		...
		{
			sprintf(szCount, "%03d", usCashOutCount);
			sprintf(szTemp, "BATCH TOTAL|CASHOUT:|%s|", szCount);
		}
		else
		{
			sprintf(szCount, "%03d", usSaleCount);
	   ...
	   strcat(szDispUI, szDisplayBuf3);
        ...
        //strcpy(szDisplayBuf0, strMMT[0].szMerchantName);
